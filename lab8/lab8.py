class CustomSet:
    ...
    def remove(self, item):
        ...
        if item not in self:
            raise KeyError("Item not in CustomSet")
        ...
        if self._len <= 1//2*self._n_buckets and 1//2*self._n_buckets>= self._min_buckets:
            self._rehash(1//2*self._n_buckets) # self._n_buckets // 2 
    ...
    def _rehash(self, new_buckets):
        ...
        new_list = [[] for i in range(new_buckets)]
        self._n_buckets = new_buckets
        for i in self._L:
            for j in i:
                new_list[self._find_bucket(j)].append(j)
        self._L = new_list[:]
